...
save.ffdf(train, dir = ffdir, overwrite = TRUE)
save.ffdf(test, dir = ffdir, overwrite = TRUE)

train = load.ffdf(dir = ffdir)
    train = train[["train"]]
    open.ffdf(train)
test = load.ffdf(dir = ffdir)
    test = test[["test"]]
    open.ffdf(test)
```
...
2.  We ascertain if there are `NA` values in the two datasets and if there are then the frequency of the `NA` across the columns.
...
3.  The first column is `id`. The values of this column don't show any discernable patterns.
