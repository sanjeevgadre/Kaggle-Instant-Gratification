...
logit.model = bigglm.ffdf(f, data = ffdfindexget(train, indx.trn), family = binomial())

prob = predict(logit.model, newdata = ffdfindexget(train, indx.tst), type = "response")
...
#prob = ff(as.numeric(NA), length = length.ff(indx.tst))
#pred = ff(as.numeric(NA), length = nrow(train))
# chunks = chunk.ffdf(ffdfindexget(train, indx.tst))
# 
...
1.  We had previously noted that `wheezy.copper.turtle.magic` is the only feature which is `integer` in class and has a very very small number of unique values (512) whereas all other features have very large number of unique values (25000+).
2.  We investigate if `wheezy.copper.turtle.magic` impacts the dependent variable. To that end split the *train* dataset along the values of `wheezy.copper.turtle.magic` and calculate and plot the class distribution for the dependent variable for each subset.
...
2.  We hypothise that separate learning models for each sub dataset would provide a significantly better fit than the single model earlier constructed for the entire dataset.
...
3.  To this end we use **Principal Component Analysis**.

```{r eda-3}
cat = unique(train$wheezy.copper.turtle.magic[])

for (i in cat[101:106]) {
  dat = train[train$wheezy.copper.turtle.magic == i, ] %>% as.data.frame()
  ...
  writeLines("")
  print(paste("For subset", i))
  print(paste("Number of components explaining 90% variance = ", which(pve < 0.9) %>% which.max()))
  print(paste("Number of components explaining 95% variance = ", which(pve < 0.95) %>% which.max()))
  print(paste("Number of components explaining 99% variance = ", which(pve < 0.99) %>% which.max()))
...
1.  The results suggest that there is significant multi-collinearity amongst features of the individual subsets of the dataset. Any learning model developed would benefit from implementing dimensionality reduction.
...
```{r eda-4}
train.vals = unique(train$wheezy.copper.turtle.magic)[]
test.vals = unique(test$wheezy.copper.turtle.magic)[]

setdiff(train.vals, test.vals)
```

*Observations*

1.  `wheezy.copper.turtle.magic` has the same set of unique values for both the *train* and the *test* dataset.
