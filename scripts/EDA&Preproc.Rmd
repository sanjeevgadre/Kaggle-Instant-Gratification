...
dat %>% ggplot(aes(Class.Prop))+geom_histogram(bins = 100)+ 
  xlab("Proportion of class =1 in the dependent variable")
...
2.  We hypothise that separate logistic regression models for each sub dataset would provide a significantly better fit than the single model earlier constructed for the entire dataset. We check this hypothesis for a few of the sub datasets.
...
for (i in cat[1:5]) {
  ...
  y = dat$target
  dat = model.matrix(target~., data = dat)[, -1]

  set.seed(i)
  indx.tst = sample(1:nrow(dat), 0.3*nrow(dat))
  
  cv.out = cv.glmnet(dat, y, nfolds = 5)
  best.lambda = cv.out$lambda.min
  
  logit.model = glmnet(dat[-indx.tst, ], y[-indx.tst], family = "binomial", lambda = best.lambda)
  prob = predict(logit.model, newx = dat[indx.tst, ], type = "response")
  labl = y[indx.tst]

  pred = prediction(prob, labl)
  perf = performance(pred, "auc")

  print(paste("The Area Under the Curve (AUC) for subset", i,"=", perf@y.values))
